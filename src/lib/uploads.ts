...
// Storage backend picks itself based on environment: Vercel Blob when
// BLOB_READ_WRITE_TOKEN is set (production, and anyone locally who opts in
// by linking a Blob store), local disk under public/uploads otherwise (the
// default for local dev, no cloud credentials required). Vercel's serverless
// filesystem is ephemeral outside /tmp, so local-disk storage cannot be used
// in production — uploaded files would vanish on the next deploy/cold start.
...
async function storeFile(
  ...
  if (USE_BLOB) {
    ...
    return { url: blob.url, mimeType: detected.mime };
  }
