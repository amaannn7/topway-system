...
import { setCustomerShares } from "../actions";
...
export function ShareList({
  ...
  initiallySharedIds: string[];
}) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [selected, setSelected] = useState<Set<string>>(new Set(initiallySharedIds));
  ...
  function save() {
    startTransition(async () => {
      try {
        await setCustomerShares(customerId, [...selected]);
